/**
 * POST /api/jobs/[jobId]/materials/resume — Generate tailored resume PDF only.
 */

import { NextRequest } from 'next/server';
import { DynamoDBWrapper } from '@worksignal/shared';
import { getAuthenticatedUser, unauthorizedResponse } from '../../../../lib/auth';
import { generateAndPersistResumeOnly } from '../../../../lib/jobMaterialsGeneration';
...
export async function POST(
  ...
  try {
    ...
    const resume = await generateAndPersistResumeOnly({
      userId: user.userId,
      jobId,
    ...
    console.error(`[materials/resume] error for job=${jobId}:`, message);
